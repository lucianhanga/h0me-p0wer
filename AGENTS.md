...
- **Savings model**: per finished day (today excluded),
  `saved = (pv_daily.to_home + cells-only discharge) × TARIFF_EUR_PER_KWH`.
  Cells-only = battery cloud day-trend positive integral (20-min power ×
  20/60/1000) **minus `pv_daily.to_home`** — the trend is the INVERTER
  output, so raw discharge + PV-to-home would double-count PV (same rule as
  the dashboard `byPeriod` accounting).
